    ...
    <div className="dg-page">
      <div className="dg-page-header">
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', margin: '4px 0 0 0', fontSize: '14px' }}>Manage users and project assignments</p>
        </div>
      </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '16px', flexWrap: 'wrap' }}>
            <div className="dg-search" style={{ flex: 1, maxWidth: '400px' }}>
              <FiSearch size={16} style={{ color: 'var(--text-muted)', position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)' }} />
              <input
                type="text"
              ...
              <tbody>
                {filteredUsers.map(user => (
                  <tr key={user.id || user._id}>
                    ...
                    <td style={{ color: 'var(--text-secondary)' }}>{user.assignedProjects?.length || 0} projects</td>
                    <td style={{ color: 'var(--text-muted)' }}>{user.lastLogin ? new Date(user.lastLogin).toLocaleDateString() : 'Never'}</td>
                    <td>
              ...
              <tbody>
                {filteredProjects.map(project => {
                  ...
                  return (
                    <tr key={project._id || project.id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                          {projectUsers.length > 0 ? (
                            ...
                            <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>No users assigned</span>
                          )}
                      ...
                      <td style={{ color: 'var(--text-muted)' }}>{new Date(project.createdAt).toLocaleDateString()}</td>
                      <td>
                        <button 
                ...
                {filteredProjects.length === 0 && (
                  <tr>
                    <td colSpan="5">
                      <div className="dg-empty" style={{ padding: '40px 20px' }}>
                        <FiBriefcase size={40} style={{ color: 'var(--text-muted)' }} />
