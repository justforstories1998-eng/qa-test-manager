...
import { FiList, FiPlus, FiSearch, FiArrowUp, FiArrowDown, FiMinus, FiEdit2, FiTrash2, FiX } from 'react-icons/fi';
import api from '../api';
...
export default function Backlogs({ projectId }) {
  ...
  const [form, setForm] = useState({ title: '', type: 'Task', priority: 'Medium', assignee: '', storyPoints: 0, description: '' });

  const fetchItems = useCallback(async () => {
    if (!projectId) return;
    setLoading(true);
    try {
      const res = await api.getWorkItems(projectId, { status: 'Backlog' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        ...
        if (res.success) { setItems(prev => prev.map(i => i._id === editItem._id ? res.data : i)); toast.success('Updated'); }
      } else {
        const res = await api.createWorkItem({ ...form, projectId, status: 'Backlog', order: items.length });
        if (res.success) { setItems(prev => [...prev, res.data]); toast.success('Added to backlog'); }
      }
      setShowModal(false); setEditItem(null); setForm({ title: '', type: 'Task', priority: 'Medium', assignee: '', storyPoints: 0, description: '' });
    } catch { toast.error('Failed'); }
  };

  const handleDelete = async (id) => {
    if (!window.confirm('Remove from backlog?')) return;
    try { await api.deleteWorkItem(id); setItems(prev => prev.filter(i => i._id !== id)); toast.success('Removed'); } catch { toast.error('Failed'); }
  };

  const moveToSprint = async (item) => {
    try {
      const res = await api.updateWorkItem(item._id, { status: 'To Do' });
      if (res.success) { setItems(prev => prev.filter(i => i._id !== item._id)); toast.success('Moved to To Do'); }
    } catch { toast.error('Failed'); }
  };

  const openEdit = (item) => { setEditItem(item); setForm({ title: item.title, type: item.type, priority: item.priority, assignee: item.assignee || '', storyPoints: item.storyPoints || 0, description: item.description || '' }); setShowModal(true); };
      ...
      <div style={{ padding: '24px 32px 16px', borderBottom: '1px solid var(--border-color, rgba(255,255,255,0.06))', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 11, background: 'linear-gradient(135deg, rgba(99,102,241,0.2), rgba(139,92,246,0.2))', border: '1px solid rgba(99,102,241,0.22)', color: '#818cf8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FiList size={19} /></div>
            <div>
              <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--text-primary, #f1f5f9)', letterSpacing: -0.3 }}>Backlog</h1>
              <p style={{ margin: '3px 0 0', fontSize: 13, color: 'var(--text-muted, rgba(148,163,184,0.55))' }}>{items.length} items</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => { setEditItem(null); setForm({ title: '', type: 'Task', priority: 'Medium', assignee: '', storyPoints: 0, description: '' }); setShowModal(true); }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg, #6366f1, #7c3aed)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}><FiPlus size={14} /> Add Item</button>
          </div>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-color, rgba(255,255,255,0.06))' }}>
                {['Title', 'Type', 'Priority', 'Points', 'Assignee', ''].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted, rgba(148,163,184,0.55))', fontSize: 13 }}>Loading...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted, rgba(148,163,184,0.35))', fontSize: 13 }}>No items in backlog</td></tr>
              ) : filtered.map(item => (
                <tr key={item._id} style={{ borderBottom: '1px solid var(--border-color, rgba(255,255,255,0.06))' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: PRIORITY_COLOR[item.priority] || '#6b7280' }}>
                      {item.priority === 'Critical' || item.priority === 'High' ? <FiArrowUp size={12} /> : item.priority === 'Low' ? <FiArrowDown size={12} /> : <FiMinus size={12} />}
                      <span style={{ fontSize: 12, fontWeight: 500 }}>{item.priority}</span>
                    </div>
                  </td>
                  <td style={{ padding: '12px 16px' }}><span style={{ fontSize: 12, fontWeight: 600, padding: '3px 10px', borderRadius: 6, background: 'rgba(99,102,241,0.1)', color: '#818cf8' }}>{item.storyPoints || 0}</span></td>
                  <td style={{ padding: '12px 16px', fontSize: 12, color: 'var(--text-secondary, rgba(203,213,225,0.85))' }}>{item.assignee || 'Unassigned'}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button onClick={() => moveToSprint(item)} title="Move to To Do" style={{ padding: '4px 8px', borderRadius: 5, border: '1px solid rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.1)', color: '#818cf8', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Sprint</button>
        ...
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 480, background: 'var(--surface-elevated, #1e293b)', borderRadius: 16, border: '1px solid var(--border-color, rgba(255,255,255,0.08))', boxShadow: '0 24px 64px rgba(0,0,0,0.4)', padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: 'var(--text-primary, #f1f5f9)' }}>{editItem ? 'Edit Item' : 'Add to Backlog'}</h3>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input value={form.assignee} onChange={e => setForm(p => ({ ...p, assignee: e.target.value }))} placeholder="Assignee" style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid var(--border-color, rgba(255,255,255,0.08))', background: 'var(--input-bg, rgba(255,255,255,0.04))', color: 'var(--text-primary, #f1f5f9)', fontSize: 13, outline: 'none' }} />
                </div>
                <div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
                ...
                <button type="submit" style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg, #6366f1, #7c3aed)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{editItem ? 'Update' : 'Add'}</button>
              </div>
            </form>
          </div>
        </div>
